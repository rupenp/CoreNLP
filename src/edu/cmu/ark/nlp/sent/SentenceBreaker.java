...
//		String rest = text.substring(last.charEnd, text.length());
    ...
    public static void main(String[] args) throws IOException {
    	for (String arg : args) {
    		if (args.length > 1)
    			System.out.printf("\nDOC\t%s\n", arg);
    		String text = GeneralUtils.readFile(arg);
    		AlignedSub textAS = ArkParser.cleanupDocument(text);
    		for (Sentence s : getSentences(textAS)) {
    			// rawText might have newlines, tabs
    			System.out.printf("SENT\t%d\t%d\nNEARRAW\t%s\n", s.charStart, s.charEnd, GeneralUtils.backslashEscape(s.rawText));
    			System.out.printf("TOKS\t%s\n", StringUtils.join(s.tokens, " "));
