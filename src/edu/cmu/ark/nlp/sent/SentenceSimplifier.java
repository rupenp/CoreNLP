...
import edu.cmu.ark.nlp.question.Question;
import edu.cmu.ark.nlp.question.QuestionUtil;
import edu.cmu.ark.nlp.question.VerbConjugator;
...
public class SentenceSimplifier {
	private boolean getComputeFeatures;
	...
	private void extractConjoinedNPs(Collection<Question> extracted, Question input) {
		...
		for(int i=0; i<nodeIndexes.size(); i++){
			...
			if(this.getComputeFeatures) newQuestion.setFeatureValue("extractedFromConjoinedPhrases", 1.0); //old feature name
			if(this.getComputeFeatures) newQuestion.setFeatureValue("extractedFromConjoinedNPs", 1.0);
	...
	private void correctTense(Tree subject, Tree clause) {
		...
		for(Tree uncle: clause.getChildrenAsList()){
			...
			if(needToModifyVerb){
				String verbLemma = QuestionUtil.getLemma(verbPreterminal.getChild(0).label().toString(), verbPreterminal.label().toString());
				String newVerb;
				//special cases
				...
				verbParent.addChild(tmpIndex, QuestionUtil.readTreeFromString("("+newVerbPOS+" "+newVerb+")"));
			}					
		}
	}
	...
	private void extractConjoinedPhrases(Collection<Question> extracted, Question input) {
		...
		while(matcher.find()){
			...
			if(this.getComputeFeatures) newTreeWithFeatures.setFeatureValue("extractedFromConjoinedPhrases", 1.0); //old feature name
			if(this.getComputeFeatures) newTreeWithFeatures.setFeatureValue("extractedFromConjoined", 1.0);
	...
	private boolean removeClauseLevelModifiers(Question q) {
		...
		if(matcher.find()){
			...
			if(this.getComputeFeatures) q.setFeatureValue("removedClauseLevelModifiers", 1.0);
			modified = true;
		}
	...
	private boolean removeVerbalModifiersAfterCommas(Question q) {
		...
		if(matchPattern.matcher(q.getIntermediateTree()).find()){
			...
			if(this.getComputeFeatures) q.setFeatureValue("removedVerbalModifiersAfterCommas", 1.0);
			return true;
		}else{
			return false;
		}
	...
	private void extractVerbParticipialModifiers(Collection<Question> extracted, Question input) {
		...
		while(matcher.find()){
			...
			Question newTreeWithFeatures = input.deeperCopy();
			newTreeWithFeatures.setIntermediateTree(newTree);
			if(this.getComputeFeatures) newTreeWithFeatures.setFeatureValue("extractedFromParticipial", 1.0);
			if(this.getComputeFeatures) newTreeWithFeatures.setFeatureValue("extractedFromVerbParticipial", 1.0);
			if(this.getComputeFeatures) System.err.println("extractVerbParticipialModifiers: "+newTree.toString());
			addIfNovel(extracted, newTreeWithFeatures);
		}
	}
	...
	private void extractSubordinateClauses(Collection<Question> extracted, Question input) {
		...
		while(matcher.find()){
			...
			Question newTreeWithFeatures = input.deeperCopy();
			newTreeWithFeatures.setIntermediateTree(newTree);
			if(this.getComputeFeatures) newTreeWithFeatures.setFeatureValue("extractedFromFiniteClause", 1.0); //old feature name
			if(this.getComputeFeatures) newTreeWithFeatures.setFeatureValue("extractedFromSubordinateClause", 1.0);
	...
	private void extractComplementClauses(Collection<Question> extracted, Question input) {
		...
		while(matcher.find()){
			...
			newTreeWithFeatures.setIntermediateTree(newTree);
			if(this.getComputeFeatures) newTreeWithFeatures.setFeatureValue("extractedFromFiniteClause", 1.0); //old feature name
			if(this.getComputeFeatures) newTreeWithFeatures.setFeatureValue("extractedFromComplementClause", 1.0);
	...
	private void extractAppositives(Collection<Question> extracted, Question input) {
		...
		while(matcher.find()){
			...
			Tree newTree = QuestionUtil.readTreeFromString("(ROOT (S "+nountree+" (VP "+copula+" "+appositivetree+") (. .)))");		
			
			addQuotationMarksIfNeeded(newTree);
			//if(GlobalProperties.getDebug()) System.err.println("extractAppositives: "+ newTree.toString());
			Question newTreeWithFeatures = input.deeperCopy();
			newTreeWithFeatures.setIntermediateTree(newTree);
			if(this.getComputeFeatures) newTreeWithFeatures.setFeatureValue("extractedFromAppositive", 1.0);
	...
	private void extractNonRestrictiveRelativeClauses(Collection<Question> extracted, Question input) {
		...
		while(matcher.find()){
			...
			if(matcherclause.find()){
				...
				Question newTreeWithFeatures = input.deeperCopy();
				newTreeWithFeatures.setIntermediateTree(newTree);
				if(this.getComputeFeatures) newTreeWithFeatures.setFeatureValue("extractedFromRelativeClause", 1.0);
				addIfNovel(extracted, newTreeWithFeatures);
			}
		}
	}
	...
	private void moveLeadingPPsAndQuotes(Question input) {
		...
		if(mainvp != null){ //any matches?
			...
			if(this.getComputeFeatures) input.setFeatureValue("movedLeadingPPs", 1.0);
		}
	...
	private void extractNounParticipialModifiers(Collection<Question> extracted, Question input) {
		...
		while(matcher.find()){
			...
			Question newTreeWithFeatures = input.deeperCopy();
			newTreeWithFeatures.setIntermediateTree(newTree);
			if(this.getComputeFeatures) newTreeWithFeatures.setFeatureValue("extractedFromParticipial", 1.0); //old feature name
			if(this.getComputeFeatures) newTreeWithFeatures.setFeatureValue("extractedFromNounParticipial", 1.0);
	...
	private boolean removeNonRestrRelClausesAndParticipials(Question q) {
		...
		if(matchPattern.matcher(q.getIntermediateTree()).find()){
			...
			ops.add(new Pair<TregexPattern,TsurgeonPattern>(matchPattern,p));
			Tsurgeon.processPatternsOnTree(ops, q.getIntermediateTree());
			if(this.getComputeFeatures) q.setFeatureValue("removedNonRestrRelClausesAndParticipials", 1.0);
			modified = true;
		}
	...
	private boolean removeParentheticals(Question q) {
		...
		if(matchPattern.matcher(q.getIntermediateTree()).find()){
			...
			if(this.getComputeFeatures) q.setFeatureValue("removedParentheticals", 1.0);
			res = true;
		}
	...
	private boolean removeAppositives(Question q) {
		...
		if(matchPattern.matcher(q.getIntermediateTree()).find()){
			...
			if(this.getComputeFeatures) q.setFeatureValue("removedAppositives", 1.0);
			return true;
		}else{
			return false;
		}
	...
	public static void main(String[] args) {
		...
		if(doMobyDickParse){
			...
			Tree mobyParse = QuestionUtil.readTreeFromString(parseStr);
			Collection<Question> output = ss.simplify(mobyParse);
			for(Question q: output){
				System.out.println(QuestionUtil.getCleanedUpYield(q.getIntermediateTree()));
		...
		try{
			...
			if(treeInput){
				...
				Tree t = QuestionUtil.readTreeFromString(doc);
			...
			while(true){
				...
				List<String> sentences = QuestionUtil.getSentences(doc);
				
				//iterate over each segmented sentence and generate questions
				List<Question> output = new ArrayList<Question>();
				
				for(String sentence: sentences){
					parsed = QuestionUtil.getInstance().parseSentence(sentence).parse;
					...
					for(Question q: output){
						System.out.print(QuestionUtil.getCleanedUpYield(q.getIntermediateTree()));
						if(verbose) System.out.print("\t"+QuestionUtil.getCleanedUpYield(q.getSourceTree()));
