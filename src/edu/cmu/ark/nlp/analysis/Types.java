...
import edu.cmu.ark.nlp.data.FirstNames;
...
//				return Gender.Neuter;
	...
	private static Gender genderByFirstNamesOrTitles(Mention m) {
		...
		Tree head = m.node().headPreTerminal(ArkParser.getInstance().getHeadFinder());
	...
	public static Number number(Mention m) {
		...
		if (isPronominal(m)) {
			...
			HeadFinder hf = ArkParser.getInstance().getHeadFinder();
			...
			// Causes data/they2  unit test to fail: Testedu.cmu.ark.nlp.testThey()
