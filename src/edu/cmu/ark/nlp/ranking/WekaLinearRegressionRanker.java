package edu.cmu.ark.nlp.ranking;
...
public class WekaLinearRegressionRanker extends BaseRanker implements IRanker {
	...
	private Classifier createClassifierObject() {
		...
		try {
			((OptionHandler) res).setOptions(opts);
		} catch (Exception e1) {
			e1.printStackTrace();
		}		
	...
	private Instance makeInstance(Instances dataset, double [] featureValues, double labelValue){
		Instance res = new DenseInstance(featureValues.length+1);
	...
	protected ArrayList<Attribute> wekaAttributes(int numFeatures) {
