...
public class DiscriminativeTagger implements Serializable {
	...
	/*
	 * feature weights are stored in an array of size equal to the number of
	 * features times the number of labels
	 * 
	 * this map goes from feature names (keys) to feature indexes WITHOUT
	 * offsets. the offsets are equal to the label index times the number of
	 * features
	 * 
	 */
	...
	private SuperSenseFeatureExtractor ssft = null;
	
	private String posTaggerModelFile;
	
	private static final String DEFAULT_POS_TAGGER_MODEL_FILE = "du/stanford/nlp/models/pos-tagger/wsj-0-18-left3words-distsim.tagger";

	public DiscriminativeTagger(Properties props) {
		featureIndexes = new HashMap<String, Integer>();
		trainingData = null;
		labels = new ArrayList<String>();
		rgen = new Random(1234567);
		this.ssft = new SuperSenseFeatureExtractor(props);
		if (props != null) {

			this.posTaggerModelFile = props.getProperty("posTaggerModelFile", DEFAULT_POS_TAGGER_MODEL_FILE);
		}else {
			this.posTaggerModelFile = DEFAULT_POS_TAGGER_MODEL_FILE;
		}
	}
	 ...
	 * @return
	 */
	public List<LabeledSentence> loadSuperSenseData(String path, List<String> labels) {
		...
		try {
			...
			while ((buf = br.readLine()) != null) {
				...
				sent.addToken(parts[0], this.ssft.getStem(parts[0], parts[1]), parts[1],
						label);
	...
	public static String removeExtraLabels(String label, List<String> labels) {
		/*
		 * if(label.contains("-adj.") || label.contains("-adv.") ||
		 * label.endsWith(".other")){ return "0"; }
		 */
	...
	public static void main(String[] args) {
		...
		if (loadFile != null) {
			...
			t = new DiscriminativeTagger(props);
	...
	*/
	...
	protected void tagStandardInput() {
		try {
			MaxentTagger met = new MaxentTagger(this.posTaggerModelFile);
			...
			while (true) {
				...
				for (String s : sentences) {
					...
					for (int i = 0; i < taggedTokens.length; i++) {
						...
						input.addToken(token, this.ssft.getStem(token, pos), pos, "0");
					}
	...
	public static DiscriminativeTagger loadModel(String loadPath) {
		...
		try {
			in = new ObjectInputStream(new GZIPInputStream(new FileInputStream(loadPath)));
		...
		for (numIters = 0; numIters < maxIters; numIters++) {
			...
			for (int i = 0; i < trainingData.size(); i++) {
				...
				if (totalInstancesProcessed % 500 == 0) {
					...
					System.err.println("pct. correct words in last 500 inst.:" + NumberFormat.getInstance()
							.format((double) (numWordsProcessed - numWordsIncorrect) / numWordsProcessed));
					numWordsIncorrect = 0;
					numWordsProcessed = 0;
				}
	...
	private void perceptronUpdate(LabeledSentence sent, double[] intermediateWeights) {
		...
		for (int j = 0; j < sent.length(); j++) {
			...
			Map<String, Double> featureValuesPredicted = this.ssft.extractFeatureValues(sent, j);
			Map<String, Double> featureValuesGold = this.ssft.extractFeatureValues(sent, j, false);
	...
	private void MIRAUpdate(LabeledSentence sent, double[] intermediateWeights) {
		...
		for (int j = 0; j < sent.length(); j++) {
			...
			tmpFeatureValues = this.ssft.extractFeatureValues(sent, j);
			...
			tmpFeatureValues = this.ssft.extractFeatureValues(sent, j, false);
		...
		for (int j = 0; j < sent.length(); j++) {
			...
			featureValuesPredicted = this.ssft.extractFeatureValues(sent, j);
			featureValuesGold = this.ssft.extractFeatureValues(sent, j, false);
		...
		for (LabeledSentence sent : sentences) {
			...
			for (String s : pred) {
				...
				if (gold.contains(s)) {
					 ...
					 * ; //System.err.println(); String [] parts =
					 * s.split("\\t"); String phrase = ""; start = new
					 * Integer(parts[1]); end = new Integer(parts[2]); for(int
					 * m=start;m<=end;m++){ phrase+=sent.getTokens().get(m)+" ";
			...
			for (String s : gold) {
				 ...
				 * //System.err.println("false negative:\t"+s+"\nPRED:\n"+sent.
				 * taggedString()+"\nGOLD:\n"+sent.taggedString(false));
				 * //System.err.println(); String [] parts = s.split("\\t");
				 * String phrase = ""; start = new Integer(parts[1]); end = new
				 * Integer(parts[2]); for(int m=start;m<=end;m++){
	...
	private void createDPTables() {
		 ...
		 * if(sent.length()>maxNumTokens){ maxNumTokens = sent.length(); } }
		 */
	...
	private void createFeatures() {
		...
		for (LabeledSentence sent : trainingData) {
			for (int i = 0; i < sent.length(); i++) {
				...
				Map<String, Double> values = this.ssft.extractFeatureValues(sent, i, false);
	...
	*/
	...
	public void findBestLabelSequenceViterbi(LabeledSentence sent, double[] weights, boolean includeLossTerm) {
		...
		// for each token
		for (int i = 0; i < numTokens; i++) {
			featureValues = this.ssft.extractFeatureValues(sent, i);
			...
			for (int j = 0; j < labels.size(); j++) {
				...
				/*
				 * Set<String> possibleSenses =
				 * SuperSenseFeatureExtractor.getInstance().getPossibleSenses(
				 * stem); Set<String> tmp =
				 * SuperSenseFeatureExtractor.getInstance().getPossibleSenses(
				 * tok); if(tmp != null && possibleSenses != null)
				 * possibleSenses.addAll(tmp); if(possibleSenses!=null &&
				 * !label.equals("0") && !possibleSenses.contains(label) &&
				 * !pos.matches("^NNPS?")){ dpValues[i][j] = maxScore;
				 * dpBackPointers[i][j] = maxIndex;
				 * if(sent.getLabels().get(i).equals(label)){ continue; }
				 * continue; }
				 */
