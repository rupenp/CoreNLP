...
public class SuperSenseFeatureExtractor {
	...
	private Map<String, Map<String, String>> senseMap; // pos, word -> most
														// frequent supersense
	private Map<String, Map<String, Integer>> senseCountMap; // pos, word ->
																// number of
																// senses
	private Map<String, Set<String>> possibleSensesMap; // stem -> set of
	...
	private String useOldDataFormat;
	private String clusterFile;
	
	private String nounFile;
	private String possibleSensesFile;
	private String verbFile;
	private String morphFile;
	private String wordNetPath;
	
	private static final String DEFAULT_CLUSTER_FILE = "data/clusters/clusters_1024_49.gz";
	private static final String DEFAULT_NOUN_FILE = "edu/cmu/ark/nlp/supersense/NOUNS_WS_SS_P.gz";
	private static final String DEFAULT_MORPH_FILE = "edu/cmu/ark/nlp/supersense/MORPH_CACHE.gz";
	private static final String DEFAULT_VERB_FILE = "edu/cmu/ark/nlp/supersense/VERBS_WS_SS.gz";
	private static final String DEFAULT_POSSIBLE_SENSES_FILE = "edu/cmu/ark/nlp/supersense/possibleSuperSenses.GAZ.gz";
	private static final String DEFAULT_WORDNET_PATH = "edu/cmu/ark/nlp/properties/jwnl_properties.xml";
	
	public SuperSenseFeatureExtractor(Properties props) {
		senseMap = null;
		morphMap = null;
		...
		this.clusterFile = props.getProperty("clusterFile", DEFAULT_CLUSTER_FILE);
		this.nounFile = props.getProperty("nounFile", DEFAULT_NOUN_FILE);
		this.verbFile = props.getProperty("verbFile", DEFAULT_VERB_FILE);
		this.morphFile = props.getProperty("morphFile", DEFAULT_MORPH_FILE);
		this.possibleSensesFile = props.getProperty("possibleSensesFile", DEFAULT_POSSIBLE_SENSES_FILE);
		this.wordNetPath = props.getProperty("wordNetPath", DEFAULT_WORDNET_PATH);
		
		this.useOldDataFormat = props.getProperty("useOldDataFormat", "true");

		String usePre = props.getProperty("usePrefixAndSuffixFeatures", "false");
		if (usePre.equals("true"))
			usePrefixAndSuffixFeatures = true;
		else
			usePrefixAndSuffixFeatures = false;

		String useClu = props.getProperty("useClusterFeatures", "false");
		if (useClu.equals("true"))
			useClusterFeatures = true;
		else
			useClusterFeatures = false;

		String useBigrams = props.getProperty("useBigramFeatures", "false");
		if (useBigrams.equals("true"))
			useBigramFeatures = true;
		else
			useBigramFeatures = false;

		String useMorphCacheStr = props.getProperty("useMorphCache", "false");
		if (useMorphCacheStr.equals("true"))
			useMorphCache = true;
		else
			useMorphCache = false;
		...
		if (!useMorphCache) {
			try {
				JWNL.initialize(new FileInputStream(this.wordNetPath));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	/**
	 * Extracts a map of feature names to values for a particular token in a
	 * sentence. These can be aggregated to get the feature vector or score for
	 * a whole sentence. These replicate the features used in Ciaramita and
	 * Altun, 2006
	 * 
	 * @param sent
	 *            the labeled sentence object to extract features from
	 * @param j
	 *            index of the word in the sentence to extract features for
	 * @param usePredictedLabels
	 *            whether to use predicted labels or gold labels (if available)
	 * @return
	 */
	public Map<String, Double> extractFeatureValues(LabeledSentence sent, int j, boolean usePredictedLabels) {
		...
		if (useClusterFeatures && clusterMap == null)
			loadClusters();
		...
		if (useClusterFeatures)
			curCluster = wordClusterID(curTok);
		...
		if (j - 2 >= 0) {
			...
			if (useClusterFeatures)
				prev2Cluster = wordClusterID(sent.getTokens().get(j - 2));
		}
		...
		if (j - 1 >= 0) {
			...
			if (useClusterFeatures)
				prevCluster = wordClusterID(sent.getTokens().get(j - 1));
		...
		if (j + 1 < sent.length()) {
			...
			if (useClusterFeatures)
				nextCluster = wordClusterID(sent.getTokens().get(j + 1));
		}
		...
		if (j + 2 < sent.length()) {
			...
			if (useClusterFeatures)
				next2Cluster = wordClusterID(sent.getTokens().get(j + 2));
		}
		...
		if (prevLabel != startSymbol)
			featureMap.put(("prevLabel=" + prevLabel), 1.0);
		...
		if (prevPOS != startSymbol) {
			...
			if (useBigramFeatures)
				featureMap.put(("prevTok+curTok=" + prevStem + "\t" + curStem), 1.0);
			featureMap.put(("prevPOS=" + prevPOS), 1.0);
			featureMap.put(("prevPOS_0=" + prevPOS.charAt(0)), 1.0);
			if (useClusterFeatures)
				featureMap.put(("prevCluster=" + prevCluster), 1.0);
			// if(useClusterFeatures)
			// featureMap.put("firstSense+prevCluster="+firstSense+"\t"+prevCluster,1.0);
		}

		if (nextPOS != endSymbol) {
			featureMap.put(("nextTok=" + nextStem), 1.0);
			if (useBigramFeatures)
				featureMap.put(("nextTok+curTok=" + nextStem + "\t" + curStem), 1.0);
			featureMap.put(("nextPOS=" + nextPOS), 1.0);
			featureMap.put(("nextPOS_0=" + nextPOS.charAt(0)), 1.0);
			if (useClusterFeatures)
				featureMap.put(("nextCluster=" + nextCluster), 1.0);
			// if(useClusterFeatures)
			// featureMap.put("firstSense+nextCluster="+firstSense+"\t"+nextCluster,1.0);
		}
		...
		if (prev2POS != startSymbol) {
			...
			if (useClusterFeatures)
				featureMap.put(("prev2Cluster=" + prev2Cluster), 1.0);
			// if(useClusterFeatures)
			// featureMap.put("firstSense+prev2Cluster="+firstSense+"\t"+prev2Cluster,1.0);
		}
		...
		if (next2POS != endSymbol) {
			...
			if (useClusterFeatures)
				featureMap.put(("next2Cluster=" + next2Cluster), 1.0);
			// if(useClusterFeatures)
			// featureMap.put("firstSense+next2Cluster="+firstSense+"\t"+next2Cluster,1.0);
		}
	...
	private String prefix(String curTok) {
		int len = curTok.length();
		if (len >= 3)
			return curTok.substring(0, 3);
		return curTok;
	}

	/**
	 * Extract most frequent sense baseline from WordNet data, using Ciaramita
	 * and Altun's approach. Also, uses the data from the Supersense tagger
	 * release.
	 * 
	 * @param sent
	 * @return
	...
	public List<String> extractFirstSensePredictedLabels(LabeledSentence sent) {
		if (senseMap == null) {
			if (this.useOldDataFormat.equals("true"))
	...
	private void loadClusters() {
		...
		try {
			...
			br = new BufferedReader(new InputStreamReader(new GZIPInputStream(new FileInputStream(this.clusterFile))));
			Integer clusterID = 0;
	...
	/**
	 * load morphology and sense information provided by the Supersense tagger
	...
	public void loadSenseDataNewFormat() {
		...
		try {
			...
			br = new BufferedReader(new InputStreamReader(new GZIPInputStream(new FileInputStream(this.nounFile))));
			while ((buf = br.readLine()) != null) {
				parts = buf.split("\\t");
			...
			br = new BufferedReader(new InputStreamReader(new GZIPInputStream(new FileInputStream(this.verbFile))));
			while ((buf = br.readLine()) != null) {
				parts = buf.split("\\t");
				String sense = parts[1].substring(parts[1].indexOf("=") + 1);
				Integer numSenses = new Integer(parts[3].substring(parts[3].indexOf("=") + 1));
			...
			br = new BufferedReader(
					new InputStreamReader(new GZIPInputStream(new FileInputStream(this.possibleSensesFile))));
			while ((buf = br.readLine()) != null) {
				parts = buf.split("\\t");
	...
	public void loadSenseDataOriginalFormat() {
		...
		loadSenseFileOriginalFormat(this.nounFile, "N");
		loadSenseFileOriginalFormat(this.verbFile, "V");
	...
	private void loadMorphDataOriginalFormat() {
		...
		try {
			...
			String buf;
			String[] parts;
			br = new BufferedReader(new InputStreamReader(new GZIPInputStream(new FileInputStream(this.morphFile))));
			while ((buf = br.readLine()) != null) {
				parts = buf.split("\\t");
	...
	public String getStemWN(String word, String pos) {
		...
		if (res.equals("is") || res.equals("are") || res.equals("were") || res.equals("was")) {
			...
			try {
				...
				// Dictionary.getInstance().getMorphologicalProcessor().lookupAllBaseForms(POS.VERB,
				// res).iterator();
	...
	private void loadSenseFileOriginalFormat(String senseFile, String shortPOS) {
		...
		if (shortPOS.equals("V"))
			spacing = 2; // verb file has 2
