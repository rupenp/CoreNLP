...
public class VerbConjugator {
	private static final String DEFAULT_VERB_CONJUGATION_FILE = "edu/cmu/ark/nlp/question/verbConjugations.txt";
	private static final String DEFAULT_JWNL_PROP_FILE = "edu/cmu/ark/nlp/properties/jwnl_properties.xml";	
	//map from lemma+pos to surface form (e.g., walk+VBZ => walks)
	private Map<String, String> conjugationMap;
	
	private Map<String, Long> baseFormCountMap;

	public VerbConjugator(Properties props){
		baseFormCountMap = new HashMap<String, Long>();
		conjugationMap = new HashMap<String, String>();
		try{
			JWNL.initialize(new FileInputStream(props.getProperty("jwnlPropertiesFile", DEFAULT_JWNL_PROP_FILE)));
			this.load(props.getProperty("verbConjugationsFile", DEFAULT_VERB_CONJUGATION_FILE));
