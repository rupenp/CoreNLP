...
public class WhPhraseGenerator {
	...
	private static final String DEFAULT_PEOPLE_PRONOUNS = "i|he|her|him|me|she|us|we|you|myself|yourself|ourselves|herself|himself";
	private static final String DEFAULT_PARTITIVE_CONSTRUCTION_HEADS = "series|set|number|part|more|all|none|rest|much|most|some|one|many|any|either|%|percent|portion|half|third|quarter|fraction|quarter|best|worst|member|bulk|majority|minority";
	
	
	public WhPhraseGenerator(Properties props, SuperSenseTagger sst){	
		...
		tokens = props.getProperty("peoplePronouns", DEFAULT_PEOPLE_PRONOUNS).split("\\|");
		for(int i=0; i<tokens.length; i++){
			peoplePronouns.add(tokens[i]);
		}
		
		tokens = props.getProperty("partitiveConstructionHeads", DEFAULT_PARTITIVE_CONSTRUCTION_HEADS).split("\\|");
		for(int i=0; i<tokens.length; i++){
	...
	public void setCurrentQuestion(Question tmp1){
		...
		if(supersenseTags == null){
			supersenseTags = this.sst.annotateSentenceWithSupersenses(tmp1.getIntermediateTree());
	...
	public void setAnswer(Tree ans, String origSentence){
		...
		answerNPHeadTokenIdx = start + leaves.indexOf(answerNP.headTerminal(this.hf));
