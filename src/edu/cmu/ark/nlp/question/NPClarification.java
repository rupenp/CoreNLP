...
import edu.cmu.ark.nlp.analysis.FindMentions;
import edu.cmu.ark.nlp.analysis.RefsToEntities;
import edu.cmu.ark.nlp.analysis.Resolve;
import edu.cmu.ark.nlp.analysis.Types;
...
public class NPClarification {
	private SuperSenseTagger sst;
	private Document doc;
	private CollinsHeadFinder hf;
	
	public NPClarification(SuperSenseTagger sst){
		this.sst = sst;
	...
	public void resolveCoreference(List<Tree> origdoc){
		...
		for(Tree t: origdoc){
			...
			trees.add(t);
			entityStrings.add(convertSupersensesToEntityString(t, this.sst.annotateSentenceWithSupersenses(t)));
		}
		
		doc = new Document(trees, entityStrings);
		
		FindMentions.go(doc);
		Resolve.go(doc, false);
	...
	private void extractClarificationFeatures(Question qCopy, List<Tree> replacedMentionTrees, List<Tree> replacementMentionTrees) {
		...
		for(int i=0;i<2;i++){
			...
			for(Tree tree: treeList){
				...
				numNPs += QuestionUtil.getNumberOfMatchesInTree("NP !> NP", tree);
				numProperNouns += QuestionUtil.getNumberOfMatchesInTree("/^NNP/", tree);
				numQuantities += QuestionUtil.getNumberOfMatchesInTree("CD|QP", tree);
				numAdjectives += QuestionUtil.getNumberOfMatchesInTree("/^JJ/", tree);
				numAdverbs += QuestionUtil.getNumberOfMatchesInTree("/^RB/", tree);
				numPPs += QuestionUtil.getNumberOfMatchesInTree("PP", tree);
				numSubordinateClauses += QuestionUtil.getNumberOfMatchesInTree("SBAR", tree);
				numConjunctions += QuestionUtil.getNumberOfMatchesInTree("CC", tree);
				numPronouns += QuestionUtil.getNumberOfMatchesInTree("/^PRP/", tree);
			}
		...
		for(int i=0; i<doc.mentions().size(); i++){
			...
			if(replacement.ID() != m.ID() && !replacement.node().dominates(m.node())){
				...
				if(isPossessiveNP(m.node()) && !isPossessiveNP(replacement.node())){
					replacementCopy.addChild(QuestionUtil.readTreeFromString("(POS 's)"));
				}
				if(!isPossessiveNP(m.node()) && isPossessiveNP(replacement.node())){
					//remove the POS node
				...
				QuestionUtil.downcaseFirstToken(replacementCopy);
	...
	private boolean isFirstMentionOfEntityInSentence(Mention mentionInOriginalDocument, Tree mentionNodeInCurrentSentence, List<Tree> mentionNodesInCurrentSentence) {
		...
		for(Mention linkedMention: doc.entGraph().getLinkedMentions(mentionInOriginalDocument)){
			...
			for(int i=0; i<mentionIndex; i++){
				if(caseInsensitiveNodeMatch(linkedMentionHead, mentionNodesInCurrentSentence.get(i).headTerminal(this.hf)))
				{
					return false;
				}
			}
	...
	public Tree simplifyMentionTree(Tree input){
		...
		Tree newHead = input.headPreTerminal(this.hf);
		...
		if(!newHead.label().toString().equals("NNP") && !newHead.label().toString().equals("NNPS")){
			...
			while(m.find()){
				np = m.getNode("np");
				Tree subtreeHead = np.headPreTerminal(this.hf);
		...
		try{
			...
			while(true){
				...
				for(String s: sentences){
					Tree t = QuestionUtil.getInstance().parseSentence(s).parse;
					trees.add(t);
