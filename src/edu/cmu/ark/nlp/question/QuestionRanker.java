...
import edu.cmu.ark.nlp.ranking.*;
...
public class QuestionRanker implements Serializable{
	...
	private BagOfWordsExtractor bow;
	private Properties props;
	private static final String DEFAULT_QUESTION_GENERATION_MODEL = "edu/cmu/ark/nlp/models/question_generation/linear-regression-ranker-reg500.ser.gz";
	
	public QuestionRanker(Properties props, String rankerType){
		this.props = props;
		this.qff = new QuestionFeatureExtractor(props);
		this.bow = new BagOfWordsExtractor(props);
		ranker = RankerFactory.createRanker(rankerType);
	}
	
	public QuestionRanker(Properties props) {
		this.props = props;
		this.qff = new QuestionFeatureExtractor(props);
		this.bow = new BagOfWordsExtractor(props);
		this.loadModel(props.getProperty("modelPath", DEFAULT_QUESTION_GENERATION_MODEL));
	}
	...
	public List<Question> loadQuestionDataWithFeatures(BufferedReader br){
		...
		try {
			while((buf = br.readLine()) != null){
				...
				//add to list
				Tree tmptree = QuestionUtil.readTreeFromString("(ROOT (. .))");
				question = new Question(this.props, tmptree);
	...
	public List<Question> loadQuestionDataWithTrees(BufferedReader br){
		...
		try {
			...
			while((buf = br.readLine()) != null){
				...
				Tree questionTree = QuestionUtil.readTreeFromString(parts[2]);
				Tree answerPhraseTree = QuestionUtil.readTreeFromString(parts[4]);
				Tree intermediateTree = QuestionUtil.readTreeFromString(parts[6]);
				Tree sourceTree = QuestionUtil.readTreeFromString(parts[8]);
				String articleID = parts[9]; 
				
				question = new Question(this.props);
				question.setLabelScore(tmpLabelScore);
				...
				this.qff.extractFinalFeatures(question);
				questions.add(question);
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void adjustScores(List<Question> questions, List<Tree> parsedSentences, boolean avoidFreqWords,
			boolean preferWH, boolean downweightPronouns, boolean doStemming)
	{

		if(avoidFreqWords){
			List<String> wordTokens = this.bow.extractNounTokensFromTrees(parsedSentences);
			Map<String, Double> typeCounts = this.bow.extractCounts(wordTokens);
