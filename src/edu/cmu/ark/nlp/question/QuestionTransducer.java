...
public class QuestionTransducer {
	...
	private boolean avoidPronounsAndDemonstratives;  //don't produce questions with pronoun
	...
	private QuestionFeatureExtractor qff;
	private VerbConjugator conjugator;
	private Properties props;

	public QuestionTransducer(Properties props, SuperSenseTagger sst){
		this.whGen = new WhPhraseGenerator(props, sst);
		this.qff = new QuestionFeatureExtractor(props);
		this.numWHPhrases = 0;
		this.props = props;
	...
	public void generateQuestionsFromParse(String inputParseStr){
		Tree inputTree = QuestionUtil.readTreeFromString(inputParseStr);
		generateQuestionsFromParse(inputTree);
	}

	
	public void generateQuestionsFromParse(Tree inputTree){
		Question q = new Question(this.props);
	...
	public void generateQuestionsFromParse(Question inputQuestion){
		...
		QuestionUtil.downcaseFirstToken(tmp1.getTree());
		...
		for(int i=0; i<numWHPhrases; i++){
			...
			if(subjectMovement){
				...
				if(this.getComputeFeatures) tmp2.setFeatureValue("isSubjectMovement", 1.0);
				if(this.getComputeFeatures) tmp2.setFeatureValue("whQuestion", 1.0);
			}else{
				tmp2.setTree(decomposePredicate(tmp2.getTree()));
				tmp2.setTree(subjectAuxiliaryInversion(tmp2.getTree()));
				if(this.getComputeFeatures) tmp2.setFeatureValue("isSubjectMovement", 0.0);
				if(this.getComputeFeatures) tmp2.setFeatureValue("whQuestion", 1.0);
			}
			...
			for(Tree t: outputTrees){
				...
				tmp2.setTree(t);
				QuestionUtil.upcaseFirstToken(tmp2.getTree());
				
				relabelPunctuationAsQuestionMark(tmp2.getTree());
				tmp2.setAnswerPhraseTree(answerPhrase);
				if(this.getComputeFeatures) this.qff.extractFinalFeatures(tmp2);
		...
		if(canInvert(tmp2.getTree())){
			...
			if(this.getComputeFeatures) tmp2.setFeatureValue("isSubjectMovement", 0.0);
			if(this.getComputeFeatures) tmp2.setFeatureValue("whQuestion", 0.0);
			if(this.getComputeFeatures) this.qff.extractFinalFeatures(tmp2);
	...
	private void relabelPunctuationAsQuestionMark(Tree inputTree) {
		...
		if(!matchPattern.matcher(inputTree).find()){
			inputTree.getChild(0).addChild(QuestionUtil.readTreeFromString("(. ?)"));
		}
	}
	...
	private Tree removeMarkersFromTree(Tree inputTree){
		...
		res = QuestionUtil.readTreeFromString(treeStr);
		return res;
	}
	...
	private List<Tree> moveWHPhrase(Tree inputTree, Tree intermediateTree, int i, boolean subjectMovement){
		...
		for(int j=0; j<whPhraseSubtrees.size(); j++){
			...
			mainclauseNode.addChild(0, QuestionUtil.readTreeFromString(whPhraseSubtree));
			...
			if(leftOverPreposition != null && leftOverPreposition.length()>0){
				prepPlaceholderParent.addChild(index, QuestionUtil.readTreeFromString(leftOverPreposition));
			}
	...
	private Tree decomposePredicate(Tree inputTree){
		...
		if(matcher.find()){
			...
			String lemma = QuestionUtil.getLemma(subtree.getChild(0).label().toString(), subtree.label().toString());		
			...
			if(!lemma.equals("be")){
				...
				tmpNode.addChild(0, QuestionUtil.readTreeFromString(aux));
				...
				tmpNode.addChild(0, QuestionUtil.readTreeFromString(lemma));
			}
		}
	...
	private String getSingularFormSubtree(Tree tensedVerbSubtree) {
		String res = "";
		String lemma = QuestionUtil.getLemma(tensedVerbSubtree.getChild(0).label().toString(), tensedVerbSubtree.label().toString());
		String pos = tensedVerbSubtree.value();
		if(pos.equals("VBD")){
			res = tensedVerbSubtree.toString();
		}else{
			res = "(VBZ "+ this.conjugator.getSurfaceForm(lemma, "VBZ") +")";
		}
		...
		if(matcher.find()){
			...
			if(subjectString.equalsIgnoreCase("I") || subjectString.equalsIgnoreCase("you")){
				...
				if(matcher.find()){
					...
					Tree singularFormSubtree = QuestionUtil.readTreeFromString(getSingularFormSubtree(verbSubtree));
	...
	public static void main(String[] args) {
		...
		try{
			...
			while((buf = br.readLine()) != null){
				if(treeInput){
					buf = QuestionUtil.preprocessTreeString(buf);
					inputTree = QuestionUtil.readTreeFromString(buf);
					QuestionUtil.getInstance().normalizeTree(inputTree);
				}else{
					if(QuestionUtil.filterOutSentenceByPunctuation(buf)){
						continue;
					}
					buf = QuestionUtil.preprocess(buf);
					if(printOriginal) System.out.println("\n"+buf);
					ParseResult parseRes = QuestionUtil.getInstance().parseSentence(buf);
