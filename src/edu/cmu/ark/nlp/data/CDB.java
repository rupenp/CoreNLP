...
public class CDB {
	...
	public CDB() {
		try {
			System.out.printf("loading CDB index/count files... ");
	...
	public FV getContextVector(String np) {
		...
		for (int i=1; i < parts.length; i++) {
			...
			if (ctx_count.length != 2) {
				System.out.println("BAD ENTRY: " + parts[i]);
				continue;
			}
	...
	public static class FV {
		...
		public String toString() {
			...
			if (keys.length > maxlen)
				ret.append(String.format("top %d of %d\n", maxlen, keys.length));
			else
				ret.append(String.format("size %d\n", keys.length));
...
//			System.out.printf("RHO %.3f\n", spearman(fv1,fv2));
			System.out.printf("%8s  %4s %4s\n", "global", "fv1c", "fv2c");
			System.out.printf("%8s  %4s %4s\n", "------", "----", "----");
			int i=0;
			for (String k : keys) {
				if (i++ > max) break;
				Integer c = I().contextCounts.get(k);
				String sc = c==null ? "null" : String.format("%.1e", c*1.0);
				System.out.printf("%8s  %4d %4d | %s\n", sc, fv1.map.get(k), fv2.map.get(k), k);
			}
		}
...
//			System.out.println(StringUtils.join(box(vals1), " "));
//			System.out.println(StringUtils.join(box(vals2)," "));
...
//			System.out.printf("Intersection size = %d\n", FV.keyIntersect(fv1, fv2).size());
			System.out.printf("jacc=%.5f ", jaccard(fv1.map.keySet(), fv2.map.keySet()));
//			System.out.printf("rho_and=%.5f ", FV.spearman_and(fv1,fv2));		
			System.out.printf("rho_or=%.5f ", FV.spearman_or(fv1,fv2));
			System.out.printf("cos=%.5f ", FV.cos(fv1,fv2));
			System.out.printf("lcos=%.5f ", FV.lcos(fv1,fv2));
			System.out.printf("\n");
		}
...
//		while(true) {
//			try {
//				System.out.printf("word1: "); String np1 = brIn.readLine();
//				System.out.printf("word2: "); String np2 = brIn.readLine();
//				if ( !I().haveNP(np1) || !I().haveNP(np2)) {
