...
import edu.cmu.ark.nlp.parse.AlignedSub;
...
public class Document implements Serializable{
	...
	private int sentenceWindow;


	public Document(Integer sentenceWindow) {
		sentences = new ArrayList<Sentence>();
		mentions = new ArrayList<Mention>();
		node2mention = new NodeHashMap<Mention>();
		refGraph = new RefGraph();
		if (sentenceWindow == null){
			this.sentenceWindow = 999;
		}else{
	...
	public static Document loadFiles(String path) throws IOException {
		...
		while ( (parseLine = parseR.readLine()) != null) {
			...
			if (parseLine.split("\t").length == 1) {
				...
				tree = ArkParser.getInstance().readTreeFromString(parseLine);
				sent.hasParse = true;
			} else {
				tree = ArkParser.getInstance().readTreeFromString(parseLine.split("\t")[2]);
	...
	public void loadSurfaceSentences(String path) throws FileNotFoundException {
		...
		int i=0;
		for (SentenceBreaker.Sentence s : ArkParser.cleanAndBreakSentences(GeneralUtils.readFile(path+".txt"))) {
	...
	public class MentionRevIterIter implements Iterator<Mention> {
		...
		public boolean hasNext() {
			...
			if (startingSentence - mNext.getSentence().ID() > sentenceWindow)
				return false;
			return true;
		}
	...
	public void doTokenAlignments(String docText) {
		System.out.println("*** Stanford <-> Raw Text alignment ***\n");
		for (Sentence s : sentences) {
			System.out.printf("S%-2d\t%s\n", s.ID(), StringUtils.join(s.tokens()));
//			System.out.println("SENTENCE WORDS     " + s.words);
//			System.out.println("" + s.surfSent);
//			System.out.println("" + s.surfSent.rawText);
			AlignedSub cleanText = ArkParser.moreCleanup(s.surfSent.rawText); 
			int[] wordAlignsInSent = ArkParser.alignTokens(cleanText.text, s.words);
