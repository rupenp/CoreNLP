...
public class AceDocument {
	...
	public String text;
	private Map<edu.cmu.ark.nlp.data.Mention, AceDocument.Mention> myMention2aceMention;
	
	public void freezeMyMentions() {
		assert myMention2aceMention==null : "freeze only once!";
		myMention2aceMention = new HashMap<edu.cmu.ark.nlp.data.Mention, AceDocument.Mention>();
		for (Mention aceM : document.getMentions()) {
			if (aceM.myMention != null)
				myMention2aceMention.put(aceM.myMention, aceM);
		}
	}
	public AceDocument.Mention getAceMention(edu.cmu.ark.nlp.data.Mention myMention) {
	...
	public static AceDocument load(String path) throws Exception {
		...
		aceDoc.text = GeneralUtils.readFile(textPath);
	...
	public static void main(String args[]) throws Exception {
		for (String f : args) {
			...
			for (Entity en : d.entities ) {
				for (Mention m : en.mentions) {
					log(m.aceID +" | "+m.ID()+" | "+m.head.charseq.text+" | "+m.extent.charseq.text);
				}
			}
		}
	}
	...
	public static class Mention {
		...
		public edu.cmu.ark.nlp.data.Mention myMention = null;
